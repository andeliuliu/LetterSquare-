...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {        
        ...
        String newWord = words[wordNum].substring(0,charNum) + letter;
        if(alreadyUsed(newWord)){
            return false;
        }
        if(!(onSameSide(lastLetter(words[wordNum]), letter)) 
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        ...
        removeLetter();
        isValid();
        iterate the charNum and wordNum (only if charNum has maxed out)
        */
        if(allLettersUsed() && dictionary.hasFullWord(this.words[wordNum]) && this.words[wordNum].length() >= 3){
        ...
        if(wordNum >= maxWords){
            ...
            for(int i = 0; i < letters.length; i++){
                if(isValid(letters[i], wordNum, charNum)){
                    ...
                    else if(dictionary.hasFullWord(words[wordNum]) && words[wordNum].length() >= 3) {
